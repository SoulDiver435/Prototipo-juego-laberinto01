...
let npcs = {
  npc1: {
    ...
    y: utils.conCelda(3),
    // direccion:"abajo",
    comportamiento: ["quieto-abajo","caminar-izquierda","caminar-izquierda","quieto-izquierda","quieto-izquierda","caminar-derecha","caminar-derecha",],
    ticks: 0,
    limiteTicks: 11,
    movRestante: 0,
    animActual: "quieto-abajo",
  ...
  npc3: {
    ...
    y: utils.conCelda(5),
    // direccion:"abajo",
    comportamiento: ["quieto-arriba", "quieto-arriba","quieto-abajo","quieto-abajo", "quieto-arriba"],
    ticks: 0,
    limiteTicks: 11,
    movRestante: 0,
    animActual: "quieto-abajo",
    frameActual: 0,
    tiempoQuieto:0,
    dialogo: ["Que emocionante.", "Este lugar es asombroso"]
  },
};
...
function obtenerDireccionNpc(propiedad, cantidad) {
...
function forzarAnimacionNpc() {
  if (mostrarMensaje && objetoCercano) {
    if (player.x > objetoCercano.x) {
      objetoCercano.animActual = "quieto-derecha";
    } else if (player.x < objetoCercano.x) {
      objetoCercano.animActual = "quieto-izquierda";
    } else if (player.y > objetoCercano.y) {
      objetoCercano.animActual = "quieto-abajo";
    } else if (player.y < objetoCercano.y) {
      objetoCercano.animActual = "quieto-arriba";
    }
  }
}
...
function actualizarSpriteNpc() {
  Object.values(npcs).forEach((npc) => {
    ...
    if (!frames.length) {
      console.error(`Error: No hay frames para la animación '${npc.animActual}'`);
      return;
    }

    let frameActual = frames[npc.frameActual] || frames[0];
    
    if (!Array.isArray(frameActual) || frameActual.length < 2) {
      console.error(`Error: frameActual es inválido para '${npc.animActual}'`);
      return;
    }

    const [xFrame, yFrame] = frameActual;
    ctx.drawImage(imgNpc1, xFrame * 32, yFrame * 32, 32, 32, xNpc, yNpc, 32, 32);
...
function comportamientoNpcs() {
  Object.values(npcs).forEach((npc) => {
    ...
    if (comportamientoActual.startsWith("quieto")) {
      añadirPared(npc.x, npc.y);
      if (npc.tiempoQuieto > 0) {
        npc.tiempoQuieto--;
      } else {
        npc.comportamiento.push(npc.comportamiento.shift()); // Cambiar al siguiente comportamiento
        npc.tiempoQuieto = 60; // Reiniciar el tiempo quieto
      }
    } else if (comportamientoActual.startsWith("caminar")) {

      if (npc.movRestante === 0) {
        npc.movRestante = 16; // Reiniciar movimiento
      }

      const [propiedad, cantidad] = movimientoNpc[comportamientoActual] || [];
      let direccionNpc = obtenerDireccionNpc(propiedad, cantidad);


      // 🚧 Si hay una pared, se queda en "quieto-dirección" y NO cambia comportamiento
      if (estaEspacioOcupado(npc.x, npc.y, direccionNpc)) {
        npc.animActual = "quieto-" + direccionNpc;
        npc.movRestante = 0;
        return; //  Evita cambiar de comportamiento
      }
    
      // Si el camino está libre, sigue caminando
      eliminarPared(npc.x, npc.y);
...
//ESTA OBJETO CERCA?
function buscarObjetoCercano(actualX, actualY, direccion){
  const {x,y}= utils.siguientePosicion(actualX, actualY, direccion);
  let npcEncontrado = Object.values(npcs).find(npc => npc.x === x && npc.y === y) || null;
...
if(!mostrarMensaje){
  objetoCercano=buscarObjetoCercano(player.x,player.y, player.direccion);
  if(objetoCercano){
    mostrarMensaje= true;
    clicksMsj=  objetoCercano.dialogo.length - 1;
    mensajeActual=  objetoCercano.dialogo[0];
    console.log("Dialogo:" + mensajeActual);
  }
}else {
  if(clicksMsj > 0){
    clicksMsj--;
    mensajeActual=  objetoCercano.dialogo[ objetoCercano.dialogo.length-clicksMsj -1];
    console.log("Dialogo: " + mensajeActual);
  }else{
...
//Cambio de Sprite de npc a la direccion del player al interactuar.
//Interaccion con Npc's con dialogo, gestion optimizada de cuadros de dialogo/texto con cualquier objeto
//Colisiones optimas de Npc's y reiteracion en intento de comportamiento
//Montaje de walls de npc y player
